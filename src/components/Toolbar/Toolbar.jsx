...
const Toolbar = ({ selectedRows, fetchData, setSelectedRows }) => {
  const handleDelete = async () => {
    try {
      ...
      const response = await axios.delete(
        "https://manageus-app.onrender.com/api/control/",
        {
          headers: {
            "auth-token": token,
          },
      ...
      if (response.status === 200) {
        ...
        console.error("Failed to delete users. Status code:", response.status);
      }
    } catch (error) {
  ...
  return (
    <div className="toolbar d-flex gap-2">
      ...
      <button type="button" className="delete link-btn" onClick={handleDelete}>
